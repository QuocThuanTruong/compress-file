...
// A portable version; it should be done in assembler
{
	...
	while (posval >= 0)
	{
		...
		posval--; bytepos--;
		if (bytepos < 0) {
			if (bytenew == 0xFF) { writebyte(0xFF); writebyte(0); }
...
void set_numbers_category_and_bitcode()
{
	...
	if (category_alloc == NULL) exitmessage((char*)"Not enough memory.");
	category = category_alloc + 32767; //allow negative subscripts
	bitcode_alloc = (bitstring *)malloc(65535 * sizeof(bitstring));
	if (bitcode_alloc == NULL) exitmessage((char*)"Not enough memory.");
...
void fdct_and_quantization(SBYTE *data, float *fdtbl, SWORD *outdata)
{
	...
	for (ctr = 7; ctr >= 0; ctr--) {
		...
		dataptr += 8;		//advance pointer to nextOfList row
	}
	...
	for (ctr = 7; ctr >= 0; ctr--) {
		...
		dataptr++;			// advance pointer to nextOfList column
	}
...
void load_bitmap(char *bitmap_name, WORD *Ximage_original, WORD *Yimage_original)
{
	...
	if (fp_bitmap == NULL) exitmessage((char*)"Cannot open bitmap file.File not found ?");
	...
	*Ximage_original = Ximage; *Yimage_original = Yimage; //Keep the old dimensions
	...
	if (RGB_buffer == NULL) exitmessage((char*)"Not enough memory for the bitmap image.");
	...
	if (tmpline == NULL) exitmessage((char*)"Not enough memory.");
